...
1. **Does `mDraftDetail` populate `inProgress` and `picks` mid-draft?** **PARTIALLY RESOLVED (2026-08-23); the remainder moved to change `add-draft-live-polling`.** Probed against the user's real ESPN account:
   - `inProgress` **does** flip to `true` while a draft room is live (the real league returned `false`, a live practice draft returned `true`). The arming signal is real.
   - A **completed** draft returns full pick data: 150 real picks, first `{overallPickNumber, roundId, roundPickNumber, teamId, playerId}` populated on every pick. Field names are confirmed.
   - **`picks` is pre-populated with a full shell array before the draft starts** — 150 entries, every `playerId: -1`. Array length is NOT a liveness signal; only `playerId != -1` counts. A naive poller would read a not-yet-started draft as complete.
   - **Practice drafts persist nothing.** With the ESPN UI showing round 6 / pick 60 / ~59 players gone, the API returned all shells at `-1` and every roster empty, across `lm-api-reads`, `kona_draft_detail`, `mLiveScoring`, the `/draftDetail` sub-path, and `kona_league_communication` (404). They are disposable and never touch real rosters.
   - **Still unknown:** whether picks appear *during* a real draft or only at completion. Only a live real draft answers it. This is now task 0.1 of `add-draft-live-polling` and gates that whole change.
...
3. ~~**What is the actual draft slot?**~~ **RESOLVED (2026-08-23).** ESPN `mSettings` exposes `pickOrder` as an array of team ids; resolving the user's own team against it gives **slot 1 of 10**. The pre-built 1.01 plan applies, but its pick numbers do not: a 10-team snake gives `[1, 20, 21, 40, 41, 60, 61, 80, 81, 100, 101, 120, 121, 140, 141]`, not the board's 12-team list.
4. ~~**Bench size**~~ **RESOLVED (2026-08-23).** ESPN `lineupSlotCounts` for the real league: `{0:1 QB, 2:2 RB, 4:2 WR, 6:1 TE, 16:1 DST, 23:3 FLEX, 20:5 BE, 21:1 IR}` — **5 bench**, and **3 FLEX rather than the board's assumed 2**. That is 10 starters, so 15 rounds. The 3-FLEX finding materially changes `flex_pressure`: 80 league-wide RB/WR/TE starters, not 54.
...
Three defects found by smoke-testing the composite against the real board, none of which the unit suite caught — every component was tested in isolation against hand-built pools, and nothing exercised `recommend()` against 161 real players at a real pick number.

- **D6a — the value term's sign was inverted.** `(adp_rank - current_pick)` rewarded candidates for being ranked *later*, and saturated the `+3.0` clamp across nearly the whole board in early rounds. At pick 1 the top five scored an identical 22.40 and the shortlist fell through to pool order. Corrected to `(current_pick - adp_rank)` — surplus, not lateness — which also makes the module sign-consistent with `qb_wait`.
- **D6b — on-the-clock tier urgency was a constant.** The divide-by-zero guard returned the maximum for *every* candidate at `picks_until_next == 0`, so the highest-weighted term contributed nothing to the ranking precisely when recommendations are requested. Now scales by tier scarcity.
- **D9a — `out_for_season` is keyword-derived and wrong**, the same defect class as `injury_tags` and more consequential, since it removes a player from the pool entirely. Nico Collins was flagged out because his note describes another player's ACL tear. Now overridden by a curated file, seeded on the observation that a genuinely-out player has also dropped out of ADP data.
